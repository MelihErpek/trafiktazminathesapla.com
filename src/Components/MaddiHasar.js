...
export default function MaddiHasar() {
  ...
  const submit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://trafikcezasorgulama.vercel.app/maddihasar",
        { aciklama, model, aracMarka, kazatarihi, kusurdurumu, telno, name }
      );
      // await axios.get(
      //   "https://trafikcezasorgulama.vercel.app/mail"
      // );
      // const response = await axios.post("http://localhost:5000/maddihasar", {  aciklama,model,aracMarka,kazatarihi, kusurdurumu,  telno,  name });
  ...
  return (
    <div>
      {/* <div className='mt-4 mb-2'>
        <select value={kazaturu} onChange={handleSelectChange}
      ...
      /> */}
      {/* <div className='my-2'>
        <select value={cinsiyet} onChange={handleSelectChangeCinsiyet}
          className="shadow  px-1 text-xs border rounded w-full  text-gray-400 leading-tight focus:outline-none focus:shadow-outline h-6"
      ...
      <input
        ...
        value={aracMarka}
        onChange={(event) => setAracMarka(event.target.value)}
      />
      <input
        className="shadow appearance-none border rounded w-full my-2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline h-6"
        id="firstName"
      ...
      <input
        ...
        id="firstName"
        type="text"
        placeholder="Meydana gelen zararı açıklar mısınız?"
        value={aciklama}
        onChange={(event) => setAciklama(event.target.value)}
      />
      {/* <div className='my-2'>
        <select value={il} onChange={handleSelectChangeIl}
          className="shadow  px-1 text-xs border rounded w-full  text-gray-400 leading-tight focus:outline-none focus:shadow-outline h-6"
